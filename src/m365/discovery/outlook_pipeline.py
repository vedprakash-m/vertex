...
class OutlookPipelineError(ValueError):
    pass


@dataclass(frozen=True, slots=True)
class OutlookPipelineBatch:
...
def run_outlook_pipeline(
    *,
    program_id: str,
    batch_id: str,
    pipeline: str = "outlook",
    from_year: int | None = None,
    programs_root: Path = PROGRAMS_ROOT,
) -> OutlookPipelineBatch:
    program = load_program(program_id, programs_root=programs_root)
    if program is None:
        raise OutlookPipelineError(f"Program '{program_id}' is missing program.yaml.")
    if program.m365 is None or not program.m365.enabled:
        return OutlookPipelineBatch(
            candidates=(),
            gaps=(
                GapDetail(
                    ...
                    window_start=None,
                    window_end=None,
                    detail=f"Program '{program_id}' has m365.enabled=false; Outlook discovery is disabled.",
                ),
            ),
        )

    workstreams = _load_workstreams(program_id, programs_root=programs_root)
    queries = _configured_outlook_queries(workstreams)
    if not queries:
        return OutlookPipelineBatch(
            candidates=(),
            gaps=(
                GapDetail(
                    ...
                    window_end=None,
                    detail=(
                        f"Program '{program_id}' has no Outlook discovery seeds in workstreams.yaml "
                        "(expected email_subject_filters, email_threads display names, or workiq_keywords)."
                    ),
                ),
            ),
        )
        ...
        return OutlookPipelineBatch(
            ...
            gaps=(
                GapDetail(
                    ...
                    window_start=None,
                    window_end=None,
                    detail="Agency CLI is unavailable, so Outlook discovery could not run.",
                ),
            ),
        )
        ...
        return OutlookPipelineBatch(
            ...
            gaps=(
                GapDetail(
                    ...
                    window_start=None,
                    window_end=None,
                    detail="WorkIQ MCP access is unavailable, so Outlook discovery could not run.",
                ),
            ),
        )

    mail_client = GraphMailClient(bridge)
    candidates: list[CandidateEvent] = []
    gaps: list[GapDetail] = []
    for query in queries:
        page = mail_client.search_emails(
            query=query,
            limit=25,
            # P4-22: allow the WorkIQ CLI fallback (workiq.exe exits code 1 on every
            # request, so allow_cli_fallback=False silently returns zero candidates)
            # and raise the timeout 45 → 300 to cover the 40-250s CLI tail latency.
            ...
            if runtime_error:
                gaps.append(
                    GapDetail(
                        ...
                        window_start=None,
                        window_end=None,
                        detail=f"Outlook discovery query '{query}' failed: {runtime_error}",
                    ),
                )
            continue
        for record in page.records:
            try:
                if _record_precedes_from_year(record, from_year):
                    continue
                candidates.extend(
                ...
                gaps.append(
                    GapDetail(
                        ...
                        window_start=None,
                        window_end=None,
                        detail=f"Outlook discovery record could not be parsed: {error}",
                    ),
                )
    return OutlookPipelineBatch(
        candidates=_collapse_outlook_candidates(candidates),
        gaps=tuple(gaps),
    )
...
def _configured_outlook_queries(workstreams: tuple[Workstream, ...]) -> tuple[str, ...]:
    queries: list[str] = []
    seen: set[str] = set()
    ...
    for workstream in workstreams:
        ...
        for subject_filter in signal_sources.email_subject_filters:
            add_query(subject_filter)
        for email_thread in signal_sources.email_threads:
            add_query(email_thread.display_name)
        for keyword in signal_sources.workiq_keywords:
            add_query(keyword)
    return tuple(queries)


def _record_precedes_from_year(record: MailRecord, from_year: int | None) -> bool:
    if from_year is None:
        return False
    recorded_at, _ = _received_at_for_record(record)
    return recorded_at.year < from_year
...
def _extract_candidates_from_record(
    *,
    ...
    identity = record.source_id or record.thread_id or record.conversation_id or "email-message"
    vault_entry = store_evidence_vault_bytes(
        program_id=program_id,
        content_bytes=payload_bytes,
        content_type="application/json",
        original_filename=f"{identity}.json",
        origin_path=record.web_url or f"outlook://{identity}",
        programs_root=programs_root,
    )
    received_at, default_temporal_confidence = _received_at_for_record(record)
    source_ref = EmailRef(
        subject=record.subject or "(untitled message)",
        sent_at=received_at,
    ...
    for raw_line in _record_marker_lines(record):
        try:
            ...
            raise OutlookPipelineError(str(error)) from error
        if parsed is None:
            continue
        ...
        occurred_at, temporal_confidence = marker_occurrence_or_default(
            parsed,
            default_occurred_at=received_at,
            default_temporal_confidence=default_temporal_confidence,
        )
        candidates.append(
            CandidateEvent(
...
def _record_marker_lines(record: MailRecord) -> tuple[str, ...]:
    lines: list[str] = []
    for value in (record.subject, record.preview):
        if value is None:
            continue
...
def _received_at_for_record(record: MailRecord) -> tuple[datetime, str]:
    raw = (record.received_at or "").strip()
    if not raw:
...
def _collapse_outlook_candidates(candidates: list[CandidateEvent]) -> tuple[CandidateEvent, ...]:
    grouped: dict[tuple[str, str], list[CandidateEvent]] = {}
    for candidate in sorted(candidates, key=_outlook_candidate_sort_key):
    ...
    for key in sorted(grouped):
        collapsed.append(_merge_outlook_candidate_group(grouped[key]))
    return tuple(collapsed)


def _merge_outlook_candidate_group(group: list[CandidateEvent]) -> CandidateEvent:
    primary = group[0]
...
def _outlook_candidate_sort_key(candidate: CandidateEvent) -> tuple[datetime, str, str]:
    message_id = getattr(candidate.source_ref, "message_id", None) or ""
    thread_id = normalize_thread_id(getattr(candidate.source_ref, "message_id", None) or "")
    return (candidate.proposed_occurred_at, message_id or thread_id or "", candidate.candidate_id)
