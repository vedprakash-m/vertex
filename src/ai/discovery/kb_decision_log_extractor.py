...
class KBDecisionLogExtractorError(ValueError):
    pass


@dataclass(frozen=True, slots=True)
class ExtractedKBDecisionBatch:
    batch_id: str
    candidates: tuple[CandidateEvent, ...]


def extract_decision_log_candidates_from_markdown(
    *,
    markdown_text: str,
    program_id: str,
    vault_hash: str,
    ...
    for line_number, raw_line in enumerate(markdown_text.splitlines(), start=1):
        ...
        try:
            ...
            raise KBDecisionLogExtractorError(str(error)) from error
        if parsed is None:
            continue
        event_type = parsed.event_type
        payload = parsed.payload
        if event_type != "decision.made.v1":
            continue
    ...
    return ExtractedKBDecisionBatch(batch_id=batch_id, candidates=tuple(candidates))


def _entity_resolution(event_type: str, payload: dict[str, object]) -> tuple[CandidateEntityResolution, ...]:
