"""Deterministic token estimation and context budgeting (Zone A).

ADF-W0.14: this module moved from ``src/ai/context_budget.py`` into ``src/core``
so the deterministic context-budget authority lives in Zone A (INV-ADF-17).
``src/ai/context_budget.py`` remains as a thin re-export shim for backward
compatibility. The optional ``tiktoken`` import pattern is acceptable in Zone A
— it is a tokenizer, not a provider SDK.

No behavior change accompanies this move; the existing test suite at
``tests/unit/test_ai_context_budget.py`` exercises the same API.
"""
