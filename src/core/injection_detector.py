"""Deterministic prompt-injection heuristic scan (Zone A).

ADF-W2.7 (Appendix B.5): moved from ``src/ai/injection_detector.py`` into
``src/core`` -- purely regex/stdlib heuristics (no provider SDK, no model
call), the same "deterministic capability, not an AI call" rationale
ADF-W0.14 already applied to token estimation. ``src/core/context_compiler.py``
(Zone A) needs this directly for Section 8.7.2 step 2 ("credential and
prompt-injection screening"); a Zone A -> Zone B import would violate
INV-ADF-17. ``src/ai/injection_detector.py`` remains as a thin re-export
shim for backward compatibility. No behavior change accompanies this move.
"""
