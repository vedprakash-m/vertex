...
| intent_router | `src/ai/intent_router.py:140` / `:137` | Yes — `intent_router.v1` | No | No | production | Used by `src/commands/ask.py`; output (a `RoutedInvocation`) directly determines which CLI command executes. Doesn't fit "published artifact/fact" cleanly — a judgment call, not asserted confidently. |
...
| m365_topic_router | `src/ai/m365_topic_router.py:105` / `:102` | Yes — `m365_topic_router.v1` | No | No | production | Used by `src/commands/gather.py`'s M365 discovery stage; feeds `promotion_candidates`/`promotion_blocked` artifact building. Borderline vs. advisory given the gather→confirm pipeline structure — noting the uncertainty explicitly. |
