...
**Version:** 1.0  
**Status:** Reflects implemented state as of 2026-07-08 — the newsletter read-path closure (`specs/fix-data-flow.md`, archived) landed on top of the 2026-07-07 REV activation proof; remaining work is operator/human-paced (`specs/bklg.md`)
**Companion specs:** [vertex-tech-spec.md](vertex-tech-spec.md) (technical specification), [vertex-ux-spec.md](vertex-ux-spec.md) (binding for visual design, layout tokens, rendering rules), `specs/bklg.md` (remaining real-data activation work)  
...
GAP-37: deferred (2026-07-26) [no-backlog-row: tracked under BL-H1]
...
BL-E1: complete (2026-07-24)
BL-E2: complete (2026-07-22)
BL-E3: complete (2026-07-27)
BL-E4: in-progress (2026-07-26)
BL-F1: complete (2026-07-24)
BL-F2: complete (2026-07-25)
BL-G1: deferred (2026-07-22)
BL-H1: deferred (2026-07-26)
BL-H2: complete (2026-07-26)
BL-H3: complete (2026-07-24)
...
| **CockpitSnapshot** | A single, versioned, read-only projection answering "what changed, what can be trusted, what did Vertex automate, what needs a human, what value was delivered, what's next" at glance/scan/deep depth for TPM/EM/operator audiences. Persisted to `programs/<id>/runtime/cockpit/latest.json` + rolling history. `vertex cockpit show\|build\|explain\|compare\|tui\|measure`. See §16.5 of `vertex-tech-spec.md`. |
| **ValueConfidence** | A four-tier honesty bar on any claimed value/time-savings figure: `measured` (real timer-based before/after data), `calibrated` (adjusted from a measured baseline), `proxy` (a heuristic estimate), `unavailable`. A "&gt;80% time saved" claim may only render as `measured` after 8+ matched `WorkflowMeasurement` pairs clear a bootstrap confidence interval — never silently inflated. |
| **AI Safety Boundary** | The mandatory pipeline every structured AI output passes through before consumption: `AISchemaGateway` (schema-only bounds check, both outbound request and inbound response) → per-feature `SemanticValidator[T]` (domain-specific checks) → a durable, fail-closed release-audit lifecycle (`AIRunState`: planned→requested→responded→schema_validated→semantically_validated, then a terminal `released\|rejected\|fallback\|discarded`) enforced by quality gate QG-29. No consumer may read an AI output that isn't durably `released`. See §12.1a of `vertex-tech-spec.md`. |
| **ContextCompiler / ContextManifest** | The deterministic, mandatory evidence-assembly stage every AI feature's prompt context must be compiled through: privacy filter → injection screening → required-evidence reservation → entity/authority filtering → dedup → salience ranking → per-source quotas → token-budget packing. Emits an auditable `ContextManifest` recording exactly what evidence was included/excluded and why. See §12.1b of `vertex-tech-spec.md`. |
| **ProgramSynthesis** | One shared, program-wide narrative synthesis (through-line, long poles, facts, inferences, source-backed recommendations) assembled deterministically from strategic risks, contradictions, critical-path milestones, and other program-level signals, then AI-authored and QG-29-released before any consumer reads it. Feeds the cockpit, the newsletter exec summary, per-workstream blurbs, `decision_brief_advisor`'s verdict generation, and `vertex brief`'s "Program Narrative" section — one contract, multiple consumer surfaces, never re-derived per surface. See §12.2b of `vertex-tech-spec.md`. |
| **OperationTrace** | A purely internal, Vertex-generated `correlation_id` linking every artifact one logical pipeline run produces (source acquisition, fact write, context manifest, AI release decision, rendered output, actuation intent/receipt) into one queryable trace. Never touches an external system; observability only, never a write blocker. See §12.2a of `vertex-tech-spec.md`. |
| **Solicitation** | A targeted, human-approved question Vertex proposes to a stakeholder when required evidence is stale or missing (e.g. no recent update on a claim nearing its due date). Never auto-sent — routed through the same nudge/Teams/email review-and-approval pipeline as any other outbound draft. A reply feeds back into re-evaluating the originating context gap. |
...
**People/Team/Program-Affiliation Registry (governed evolution of the shape above, fully closed as of 2026-07-27):** the alias-keyed shape above has been superseded by a governed, `EntityRegistry`-integrated People/Team/Program-Affiliation Registry — stable canonical entity IDs (not alias), typed temporal team membership, field/relation-level provenance and freshness, a pluggable identity-provider port, and safe transactional shared writes. The registry extends rather than replaces this section's mechanisms; alias-keyed reads continue to work unchanged throughout the migration window (schema 2.x); the schema-3.0 hard-removal horizon is defined and measured, not open-ended — zero live legacy-alias reads across 8 consecutive weeks of real usage (instrumentation live since 2026-07-22, so the earliest possible date the horizon can report `met` is 2026-09-16). Every phase is complete: durable shared-store foundation, identity/migration, query/doctor (including scale-envelope and scoped/lazy-loading work that cut full-doctor time at a hypothetical 10,000-person/100-program fleet-scale fixture by ~84%, still short of the 10s budget at that scale — see `specs/bklg.md` BL-E2; the currently-supported, currently-met profile is 1,000 people/100 teams/20 programs, ~5.8x headroom over real usage), provider refresh, audience/delegation, and material ledger projection — `team.membership.changed`, `ownership.changed`, and `identity.lifecycle_changed` all register and emit for affected active programs via a steward-authorized `vertex kb people lifecycle-set` transition path. A full `DIR-*` doctor-check contract (schema/integrity, freshness, lifecycle, shadow-file hygiene, provider health, audience scope, cache, manifest-integrity) covers the shared registry end to end. The graded PII policy (365-day retention; `sensitive_only`-required/`all`-recommended encryption; an always-required reveal-authorization allowlist, workspace-global) was decided and shipped 2026-07-24. **The real operator-run onboarding pilot with live DSAR/rollback proof — the one gate no coding session could close on its own — ran for real 2026-07-27**: a person was onboarded through the production path into the live registry, and backward-compatibility, backup/restore, a real audited DSAR export, a real DSAR forget (verified as genuine redaction/tombstoning against the actual files, not just a query-side filter), and a rollback-from-backup recovery were each evidenced with real command output (`governance/runbooks/ppl-w64-onboarding-pilot-runbook.md`). Every Phase 6 gate is now closed. Beyond the original spec's own scope, a demand-driven freshness/enrichment cadence now runs on top of the registry: fields that are stale or were never verified become WorkIQ-question candidates for real stakeholders only, review is always human-in-the-loop (a WorkIQ answer is never auto-applied), and the cadence itself is tied to Vertex's own operational rhythm (every Nth completed report/nudge run) rather than an OS-level scheduler. The original spec is archived to `.archive/specs/people.md`; see that document for the full binding contract and work-item ledger, and `specs/bklg.md` §9 for the closure evidence.
...
### 9.28 Cockpit (`vertex cockpit`)

**Purpose:** One unified reading surface answering, at three depths (glance ≤10s / scan ≤60s / deep 2-8min) for three audiences (TPM, EM, platform operator): what changed, what can be trusted, what did Vertex automate on my behalf, what needs a human decision, what measurable value was delivered, and what's the single next action.

**Composition:** a `CockpitSnapshot` (see Glossary) aggregates program status, source health, AI-generated intelligence (release-gated `ProgramSynthesis` through-line, when one exists), operational economics (AI spend/tier routing), earned autonomy/trust state per proposal class, and time-savings value — each built by its own best-effort reader so one subsystem's failure never blanks the whole cockpit.

**Value measurement is honesty-gated**, not aspirational: a workflow may only claim a `measured` time-savings figure after 8+ real, timer-based before/after `WorkflowMeasurement` pairs clear a bootstrap statistical confidence interval (`TimeSavingsCertification`). Below that evidence bar, value renders as `proxy` or `calibrated`, explicitly labeled — never silently presented as a hard number a leader could cite without caveat.

**Outcome metrics OM-1/2/4/5** (`governance/outcome-metrics.md`, `src/core/outcome_metrics.py`, shipped 2026-07-22) apply the same honesty discipline to the platform's own safety/reliability claims, not just time-savings: OM-1 (zero hallucinated facts reach a published newsletter, 8-week window), OM-2 (zero duplicate ADO entities created, 6 months), OM-4 (zero unaudited AI outputs consumed), and OM-5 (a weekly cycle completes with zero forced overrides in steady state) are the platform's own MVP acceptance bar for the reliability canary window (§9.7). Each carries an explicit confidence tier (`measured`/`unavailable`) surfaced in `vertex cockpit`/`vertex doctor` — a tier is never silently promoted. As of the last measurement, OM-4 is `measured` (real audit-ledger coverage) while OM-1/2/5 are honestly `unavailable`, each for a stated, structural reason (OM-1 needs the AI safety boundary to cover the REV extractor/judge path; OM-2 needs a live ADO mutation domain to correlate against; OM-5 needs a pre-registered "what is timed, for whom, against what baseline" protocol) — see the governance doc for current status and what each metric needs to become measurable.

**Interactive review surface:** `vertex cockpit tui` is also the review-and-launch point for every human-reviewable AI proposal type (risk promotions and all AISchemaGateway-generated proposal classes) — approve/reject a staged proposal without leaving the cockpit.

**Acceptance criteria:**
1. `vertex cockpit show --program <id>` renders a snapshot (terminal or `--format html`) reflecting the program's current state within the last `build` cycle.
2. A subsystem failure (e.g., no released synthesis yet, a source degraded) degrades that section to an honest empty/default state, never a crash or a silently stale figure.
3. `vertex cockpit explain <finding_id>` traces any surfaced finding back to its underlying evidence.
4. No value metric renders as `measured` without a certified `TimeSavingsCertification` behind it.

### 9.29 Gap-Driven Solicitation

**Purpose:** When required evidence is stale or missing for a claim, decision, or milestone nearing its due date, Vertex may propose — never auto-send — a targeted, specific question to the right stakeholder, closing the loop that today requires a PM to notice the gap and chase it manually.

**Flow:** context gap detected → eligibility/cooldown check (no repeat-pestering the same person) → recipient/question/channel proposed → human approves or edits → sent through the existing nudge/Teams/email outbox (same audience-policy guardrails as any other outbound draft) → a reply is ingested and feeds back into re-evaluating whether the gap is closed.

**Non-goal:** this is not a scheduled/daemon capability. Cadence comes from the operator invoking `vertex prefetch` / `vertex cockpit build` / `vertex alerts deliver` on their own schedule (an OS-level scheduler such as Task Scheduler or cron, if desired) — Vertex itself never runs as a background service.

**Acceptance criteria:**
1. No solicitation is ever sent without an explicit human approval step, matching every other outbound-draft surface's contract.
2. A stakeholder is never solicited more than once within the configured cooldown window for the same gap.
3. A reply to a solicitation is ingested and visibly changes the originating gap's state (closed, partially answered, or still open).
...
3. **Routine review shrinks over time** — consistently-approved signals flow automatically (tracked longitudinally as fleet scale grows; see `specs/bklg.md` §4 non-goals).
...
- **No unbounded/unaudited AI context:** every structured AI feature's prompt context is assembled through the mandatory `ContextCompiler` (§12.1b, `vertex-tech-spec.md`) — no AI generator may assemble context ad hoc. Every structured AI output passes through the fail-closed AI Safety Boundary (§12.1a) before any consumer may read it; nothing bypasses the QG-29 release-audit gate to force-consume an unreleased result.
- **Full-chain observability:** every pipeline run (report, gather, rev) carries one internal correlation id (`OperationTrace`, §12.2a) linking source acquisition through fact write, context compile, AI release decision, rendered output, and actuation receipt — enabling end-to-end traceability of any published claim back to its originating evidence.
...
- `governance/privacy-matrix.md` — data-classification matrix, GDPR/CCPA applicability, retention policies, and DPA review checklist (WS-15; DPA review is a `[HUMAN GATE]` in `specs/bklg.md`)
...
**Two-lane evaluation harness:** promotion/demotion evidence is gathered through two separated lanes, per `(program_id, proposal_class)`. A **CI regression lane** gates every code/prompt merge against a synthetic fixture corpus — fast, deterministic, always-on. A **human-holdout lane** gates enforce-mode promotion itself — real labeled outcomes from a human reviewer, never a synthetic fixture. An LLM-as-judge may assist the holdout lane's diagnostics (surfacing likely-mislabeled cases for human review) but is never treated as sole ground truth for a promotion decision — only a human holdout label counts. `vertex cockpit`'s trust view (§9.28) surfaces current level, precision, acceptance rate, and promotion evidence per proposal class.
