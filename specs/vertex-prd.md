...
**People/Team/Program-Affiliation Registry (governed evolution of the shape above, complete as of 2026-07-21):** the alias-keyed shape above has been superseded by a governed, `EntityRegistry`-integrated People/Team/Program-Affiliation Registry — stable canonical entity IDs (not alias), typed temporal team membership, field/relation-level provenance and freshness, a pluggable identity-provider port, and safe transactional shared writes. The registry extends rather than replaces this section's mechanisms; alias-keyed reads continue to work unchanged throughout the migration window (schema 2.x), with hard removal only at a schema-3.0 horizon gated on zero live legacy references. Every agent-executable phase is complete: durable shared-store foundation, identity/migration, query/doctor (including scale-envelope and scoped/lazy-loading work that cut full-doctor time at a hypothetical 10,000-person/100-program fleet-scale fixture by ~84%, still short of the 10s budget at that scale — see `specs/bklg.md` BL-E2; the currently-supported, currently-met profile is 1,000 people/100 teams/20 programs, ~5.8x headroom over real usage), provider refresh, audience/delegation, and material ledger projection — `team.membership.changed`, `ownership.changed`, and `identity.lifecycle_changed` all register and emit for affected active programs via a steward-authorized `vertex kb people lifecycle-set` transition path. A full `DIR-*` doctor-check contract (schema/integrity, freshness, lifecycle, shadow-file hygiene, provider health, audience scope, cache, manifest-integrity) covers the shared registry end to end. Two items remain, neither agent-closeable: a real operator-run onboarding pilot with live DSAR/rollback proof (the only Phase 6 gate), and a DRI-decision-gated graded PII retention/encryption/reveal policy — both tracked in `specs/bklg.md` §9. The original spec is archived to `.archive/specs/people.md`; see that document for the full binding contract and work-item ledger.
