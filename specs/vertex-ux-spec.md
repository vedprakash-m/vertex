...
**Current surface posture (2026-06-09):** Weekly Acme HTML, Teams/Markdown, reviewer pane, nudge EML, freshness HTML, CLI, proposal review, and daily dry-run surfaces are implemented. `acme_daily` renders with data/config warnings rather than code failures. `acme_nudge` dry-run generated an owner-ready consolidated EML. `acme_lt_deck` needs a rerun because the last online dry-run was interrupted mid-fetch; `acme_quarterly` is gated on enough confirmed weekly issues. The open UX work is therefore validation and operator adoption rather than new layout primitives. Accessibility automation (alt text, table semantics, contrast) is done, CI-enforced, and closed per [backlog.md](backlog.md) BL-H2 (2026-07-26); manual Outlook/mobile per-client evidence capture and a formal independent audit were deliberately closed as an accepted limitation at Vertex's current internal, single/dual-program scale rather than left open — see BL-H2 for the named reopen trigger.
...
- No formal §17 acceptance-mapping ID exists for this surface yet. Golden-snapshot goldens shipped (`tests/golden/test_cockpit_html_golden.py`). A real automated WCAG scan (axe-core, 2026-07-26, `specs/bklg.md` BL-H2) found and fixed one real dark-mode contrast bug (`code` element); a full *independent* (human-conducted) audit was deliberately not pursued — no external party checks this claim at Vertex's current single/dual-program scale, so it is closed as an accepted limitation with a named reopen trigger (customer-facing surface, compliance ask, or fleet growth) rather than left open indefinitely.
