...
**Version:** 1.0  
**Status:** Reflects implemented state as of 2026-07-08 (newsletter read-path closure, `specs/fix-data-flow.md`, archived, landed on top of the 2026-07-07 REV activation proof); remaining work is operator/human-paced, tracked in `specs/bklg.md`
**Companion docs:** [vertex-prd.md](vertex-prd.md) (requirements), [vertex-ux-spec.md](vertex-ux-spec.md) (visual design), `specs/bklg.md` (remaining real-data activation work), `.archive/specs/transform.md` (V2 transformation history, archived)  
...
- Last updated: 2026-07-07 (session 2) — Reconciled `specs/bklg.md`'s two remaining operator-paced items with direct evidence. **BL-A1 (Azure Content Safety):** provisioned and verified live (a real shield call returned a genuine `clean` verdict); the remaining 5-consecutive-clean-cycle gate is now purely wall-clock, not a config/engineering gap. **BL-A2 (corpus certification):** corrected an earlier miscalibration — `reachable_document_count=72` measured documents that might mention the keystone family broadly, not real extraction-sourced yield of the specific certifying claim; a direct candidate-store audit found only 1 real extraction-sourced `milestone.completed` instance (and single digits for the other two accessor families) across the pilot's entire real history, with Wilson-CI math confirming ≥25 such instances are needed even with perfect labels. `recommended_v1_authoritative` is accepted as the durable operating tier for single-program deployments; full statistical certification is deferred until fleet rollout (≥3 programs) makes pooled dual-annotation viable. Updated §13.6.5's operator-gate summary accordingly.
- Last updated: 2026-07-07 — Incorporated `specs/activation.md` (ACTIVATION-1 v1.0→v1.29, archived to `.archive/specs/activation.md`) into §13.6. Added §13.6.7 documenting the real-data activation proof (the activation sentence fired for the first time on a real pilot program's data), five previously-undetected code gaps found and fixed on first real contact (candidate-store schema drift, milestone stub `target_date`, the fact↔record entity-ref join, `source_document_key` bridge wiring, and the counterfactual-proof harness), the read-time `approval_event_id` reverse-lookup join, the hardening contracts (`is_clean_cycle()`, cycle classes, the composite privacy gate, entity-resolution wiring, cross-source conflict detection), and the `scripts/verify_activation.py` self-verification tool. Added REV-25 to the implementation-status table. Replaced the S-9e/Q7/S-10a operator-gate summary with a pointer to `specs/bklg.md`, which now carries the two remaining real-data items (Azure Content Safety provisioning; corpus dual-annotation) as an executable-gate-mapped feature spec.
...
| Knowledge plane | `src/core/knowledge/predicate_registry.py` (closed claim-predicate vocabulary); claim store integrated with `src/core/knowledge_claim_store.py` |
...
- `src/core/knowledge/predicate_registry.py` — closed vocabulary of claim predicates (e.g., `depends_on`, `is_deprecated`, `owner_changed`, `risk_accepted`); unknown predicates are rejected at claim-write time (QG-DM-12 variant).
...
Foundational persistence primitives closing the architecture-remediation program's Phase 0/1 (see `.archive/specs/arch-fix.md` for the full audit trail and `specs/bklg.md` §7 for remaining Phase 2/2b/3 work). These exist as reusable Zone A infrastructure — they are not yet wired into any AI-safety or actuation call path (that wiring is the remaining `arch-fix` work).
...
**People/Team/Program-Affiliation Registry (governed evolution of the shape above, complete as of 2026-07-21):** built out on `EntityRegistry`-integrated typed files (`entities.yaml` schema 2.0, `people_directory.yaml`/`teams.yaml` schema 2.0, `memberships.yaml`/`delegations.yaml` schema 1.0), a workspace-global fenced-lease staged-transaction writer, and a field-level append-only change journal — see `.archive/specs/people.md` for the full binding data/behavior contract and work-item ledger. Reads go through a set of typed query functions (`src/core/people_query.py`: `find_person`, `find_team`, `team_members`, `search_people`, `list_stale_people`, `list_conflicts`, `find_registry_program_affiliations`), not a single compiled read-model class. `ProgramReality` still loads `EntityRegistry` directly for entity resolution rather than consuming the newer registry query layer; consolidating those onto one injected read model is not a named work item. Scoped/lazy per-program loading (the load contract of `load_people_directory`/`load_teams`/`load_entities_document`/`read_all_memberships`) shipped and cut full-doctor time at a hypothetical 10,000-person/100-program fleet-scale fixture from 704.656s to a 111-134s noisy range (~81-84%) — still an 11-13x miss against the 10-second budget at that scale, with the remaining gap converging on a genuine one-time full-parse cost a caching/loader-swap fix structurally cannot remove. **BL-E2 (resolved 2026-07-22, `specs/bklg.md` §9):** rather than leave that miss open indefinitely, the publicly-supported scale was narrowed to what is actually met. The **currently-supported profile is 1,000 people / 100 teams / 20 programs** — roughly 5.8x/4.3x/2.2x headroom over real current usage (XPF, 172 people / 23 teams / 9 programs) — empirically measured (not interpolated) at 6.3-6.6s against the 10s budget, hard-enforced by `tests/perf/test_people_registry_scale.py::test_registry_scale_envelope_full_doctor_current_scale`. The original 10,000-person/2,000-team/100-program "Target/local SSD" profile remains documented as a **future fleet-scale aspiration** (contingent on `specs/bklg.md` BL-G1's fleet-rollout decision, which has no sponsor or timeline yet), not a currently-met commitment; that profile's own scale-envelope tests stay in place as `xfail(strict=False)`, re-measured on every CI run so an eventual persistence-format fix (SQLite-backed scoped/lazy loading) can flip them to a real pass. A full `DIR-*` doctor-check contract covers schema/integrity, freshness, lifecycle, shadow-file hygiene (program-local `people_directory.yaml`/`teams.yaml`/`entities.yaml` records are actively cleared once migrated into the shared root, and separately detected if they aren't), provider health, audience scope, cache, and manifest integrity. **DIR-08A/08B (graded PII retention/encryption/reveal policy) shipped 2026-07-24** (`specs/bklg.md` §9 BL-E1): 365-day retention, `sensitive_only`-required/`all`-recommended encryption, an always-required reveal-authorization allowlist, workspace-global; the DIR-08 check enforces both halves (DIR-08A structural, DIR-08B the actual-vs-required encryption posture) — the current real posture is plaintext storage for `people_profiles.yaml`, an accepted, explicitly tracked gap pending real profile-data volume, not a silent miss. 8 synthetic example templates exist at `knowledge/*.example.yaml` illustrating the schema. Persistence remains YAML-authoritative for v1 (accepted resolution, `.archive/specs/people.md` §5.5) with an evidence-gated re-evaluation trigger, not an immediate SQLite migration.

**Demand-driven freshness/enrichment cadence (`specs/bklg.md` BL-E4, shipped 2026-07-26):** `src/core/people_enrichment.py` (Zone A) selects enrichment candidates — a field is a candidate only if its person is a real stakeholder in a program's *current* committed state and the field is either stale (past `freshness_policy.yaml`'s `people_registry.stale_after_days`, default 90) or was never verified and is currently empty. `vertex kb people enrich` (Zone C orchestration) asks WorkIQ one targeted question per candidate, capped per run given WorkIQ's 36-180s per-call latency, and records every answer as a `pending` event in an event-sourced ledger (`programs/<id>/_quality/people_enrichment_candidates.jsonl`) — never auto-applied. `enrichment-list`/`enrichment-resolve --decision accept|reject` close the human-in-the-loop loop; only `accept --apply` writes, through the same staged `apply_shared_registry_patch` writer every other registry mutation uses. The trigger itself is event-driven, not a wall-clock schedule: `freshness_policy.yaml`'s `people_registry.enrichment_trigger` (`nudge_run_every`/`report_run_every`, default 5/3) ticks a counter on every completed nudge or report run and raises a between-runs alert (the same NG-3 no-daemon mechanism `gather`/`confirm`/`doctor` already use) once the threshold is crossed — `report.py` never calls WorkIQ inline itself (INV-ADF-2 is unaffected). A real run against live WorkIQ (2026-07-26) proved the loop end-to-end and correctly rejected every answer that came back `[Unverified]` (the binding constraint is Microsoft IT's Graph API directory-read block, not a defect in this mechanism).
...
| `slice_contracts.yaml` | Per-dimension: source_of_truth, ADO assignment, freshness SLAs, degradation templates. Schema `1.1` (backward-compatible with `1.0`, complete as of 2026-07-22 — `.archive/specs/armada.md`) adds `workstream_id`, per-binding query-consumption `mode`, `workstream_ids[]`/`lane_ids[]` attribution, and a closed filter-operator set; see §11.3 |
...
**Gather-run manifests (`gather-run.v1`, complete as of 2026-07-22 — see `.archive/specs/armada.md` for the full binding contract and work-item ledger):** an opt-in, program-configured activation layer (`GatherActivationConfig`, `off`/`shadow`/`enforce`, parsed and enforced by `gather_pipeline/lifecycle_policy.py`) wraps `gather_program()` with a versioned run-lifecycle state machine — `staging` → `committed`/`failed`/`quarantined` — implemented in `src/core/gather_run_manifest.py`. Staged artifacts (`ado_items.jsonl`, `query_results.json`, channel outcomes) are written under a run-scoped directory and promoted atomically (`os.replace`, with bounded retry on transient Windows handle-release failures) only once a run commits; `latest.json`/`latest_full.json` pointers are updated atomically last, so no reader ever observes a partially-written run. Each manifest carries a canonical content hash, per-query capture/skew timestamps, discovered/hydrated global unique counts plus per-query raw counts, a resolved completeness-oracle result (`resolve_oracle_result()`; order-2 operator-recorded source-export reconciliation via `vertex gather --source-export <scope_id>=<count>`, order-1 independent Kusto/OData oracle intentionally deferred pending qualification evidence), shrinkage classification, and freshness state (next-expected-full timestamp, current/warn/block thresholds). `report.py`, draft state, and `confirm.py` all resolve and pin the same committed manifest once per run rather than re-fetching membership. A one-time `create_legacy_cutoff_manifest()` / `vertex admin bootstrap-legacy-cutoff --program <id>` bootstraps a synthetic cutoff manifest so `journal.read_signals()` and `program_fact_store.load_program_facts()` can safely bound pre-activation, unstamped record pass-through once a program flips `enforce`; absent a cutoff, today's unconditional pass-through is preserved exactly (full backward compatibility). Retention (`privacy_purge`) removes expired non-current committed/failed/quarantined run directories (90-day default) while always preserving pointer-named and confirm-bound runs; `observability bundle` emits a redacted `gather_runs.json` summary excluding raw delivery-scope membership. A workspace-scoped lease/fencing mechanism serializes concurrent gather attempts, and `gather_schedule_monitor` opens/resolves a `gather_missed_attempt` alert independent of data freshness. Slice-contract schema `1.1` (`src/core/slice_contract_loader.py`, backward-compatible with `1.0`) adds `workstream_id`, per-binding `mode` (`full_scope`/`activity_delta`/`analytics_history`), `workstream_ids[]`/`lane_ids[]` multi-lane attribution, and a closed filter-operator set (`area_path`, `work_item_type`, `title`, `tag`); `report_fetch.py` partitions saved-query bindings by mode so `full_scope` bindings are never date-bounded while `activity_delta` bindings retain the existing recency window, and `analytics_history` bindings never render as current scope. Golden queries (`knowledge/golden_queries.yaml`) carry a closed `classification` (`validation`/`analytics_history`/`evidence`/`hygiene`/`retired`) so no golden query can be mistaken for authoritative delivery scope.

**Tag-hygiene leakage-rate metric (`specs/bklg.md` BL-F1, shipped 2026-07-24):** any program that configures an `ado_odata`-engine `hygiene`-classified golden query gets a `leakage_rate = likely_missing_tag_items / max(authoritative_scope_items + likely_missing_tag_items, 1)` measurement, computed by `src/core/armada_leakage.py`'s `compute_leakage_rate()` against `resolve_latest_committed_manifest()`'s authoritative-scope count. Candidates are tracked in an event-sourced ledger (`programs/<id>/_quality/leakage_candidates.jsonl`) with first-seen/reseen/reopened/auto-resolved lifecycle transitions, a 7-day owner-disposition and 14-day no-unresolved-candidate SLA clock (measured from `first_seen_at`, never paused), and a `correctly_untagged` disposition that durably suppresses a false positive from future counts. Surfaced as a warn-only `vertex doctor --storage` check (never blocks) — the metric is a longer-horizon certification trend target, not a per-run gate. Validated against a real live tenant (2026-07-24): 64 real candidates found, `leakage_rate=0.6957` measured with `confidence=measured`, correctly non-blocking since no SLA clock had yet elapsed.
...
The per-feature AI Safety Boundary named in `.archive/specs/arch-fix.md` (originally `AF-1`/`AF-2`/`AF-3`) was built and shipped as part of the `arch-data-fix` program (see `.archive/specs/arch-data-fix.md` ADF-F09/ADF-W2.8). **Corrected 2026-07-22 (specs/bklg.md BL-C2):** this section previously claimed the boundary was "live across every structured AI generator," naming only 6 features — a 2026-07-21 audit found that claim overstated (the REV extractor/judge path and several other production call sites bypassed the boundary entirely) and reopened BL-C2 as a full coverage epic. That epic is now closed: every `production`-classified call site in `governance/ai-call-inventory.md` (`risk_proposal_generator`, `decision_brief_advisor`'s baseline path, `program_synthesizer`'s production path, `top_three_candidate_generator`, `governance_decision_brief_generator`, `dependency_blast_radius_generator`, `blurb_generator`, `claim_extractor`, `exec_summary_drafter`, `summary_generator`, `synthesizer`, `intent_router`, `m365_topic_router`, `lookback_retrospective`, and `anticipation_engine`) runs the full bounds-check → semantic-validation → QG-29 lifecycle sequence below, verified by direct code read and a ratchet test (`tests/contracts/test_ai_call_site_ratchet.py`) rather than re-asserted from this row's own prior text. **One stated, honest exception**: `anticipation_engine`'s `program_id is None` branch (used by `review_full.py`, which can legitimately run without a resolved program) applies bounds/semantic validation but cannot write a durable QG-29 audit record — there is no program to attribute one to. `advisory`/`evaluation`-classified sites (e.g. `decision_brief_advisor`'s pilot path, `program_synthesizer`'s evaluation path, `rev_judge`) intentionally carry a reduced bar per the inventory's own classification, not a gap.
