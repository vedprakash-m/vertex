...
**Version:** 1.0  
**Status:** Reflects implemented state as of 2026-07-07 (REV activation proof landed; remaining work is operator/human-paced, tracked in `specs/backlog.md`)
...
- **BL-A2 (corpus certification, was S-9e):** recruit a second annotator for the ≥20-doc κ dual-label set (κ ≥ 0.7), grow the extraction population, and freeze the train/dev/test split. As of 2026-07-07, `reachable_document_count` for the keystone family is 72 (raw-data acquisition is solved — see REV-25); 0 are labeled/dual-labeled. This is the only remaining trust blocker for G-corpus/G-floor certification and Q7.
- **Q7 (production extractor):** no promotion decision possible until BL-A2 certifies. The deterministic extractor already clears G-floor (G-xtract-prec 86.7%, G-accept-prec 100%) on preliminary data, so the LLM is not required for the precision floor; the LLM deployment (`gpt-5.4-mini`) is separately confirmed working on real mail as of REV-25.
- **BL-A1 (Azure Content Safety, was S-10a):** IT provisioning of Azure CS + `VERTEX_AI_JUDGE_DEPLOYMENT` (a different underlying model than `VERTEX_AI_DEPLOYMENT`). Blocks `is_clean_cycle()`/the AG-3 authority-ladder gate (§13.6.7), not the AG-1 activation proof itself, which does not require a non-degraded cycle.
