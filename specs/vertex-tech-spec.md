...
**People/Team/Program-Affiliation Registry (governed evolution of the shape above, complete as of 2026-07-21):** built out on `EntityRegistry`-integrated typed files (`entities.yaml` schema 2.0, `people_directory.yaml`/`teams.yaml` schema 2.0, `memberships.yaml`/`delegations.yaml` schema 1.0), a workspace-global fenced-lease staged-transaction writer, and a field-level append-only change journal — see `.archive/specs/people.md` for the full binding data/behavior contract and work-item ledger. Reads go through a set of typed query functions (`src/core/people_query.py`: `find_person`, `find_team`, `team_members`, `search_people`, `list_stale_people`, `list_conflicts`, `find_registry_program_affiliations`), not a single compiled read-model class. `ProgramReality` still loads `EntityRegistry` directly for entity resolution rather than consuming the newer registry query layer; consolidating those onto one injected read model is not a named work item. Scoped/lazy per-program loading (the load contract of `load_people_directory`/`load_teams`/`load_entities_document`/`read_all_memberships`) shipped and cut full-doctor time at true 10,000-person/100-program scale from 704.656s to a 111-134s noisy range (~81-84%); the remaining gap to the 10-second target budget converges on a genuine one-time full-parse cost that a caching/loader-swap fix cannot remove and is evidence-gated behind the persistence ADR's own re-evaluation trigger (`specs/backlog.md` §9, BL-E2), not open engineering work. A full `DIR-*` doctor-check contract covers schema/integrity, freshness, lifecycle, shadow-file hygiene (program-local `people_directory.yaml`/`teams.yaml`/`entities.yaml` records are actively cleared once migrated into the shared root, and separately detected if they aren't), provider health, audience scope, cache, and manifest integrity; the one still-unbuilt code, DIR-08A/08B (graded PII retention/encryption/reveal policy), needs a DRI policy decision and is tracked as `specs/backlog.md` §9 BL-E1. 8 synthetic example templates exist at `knowledge/*.example.yaml` illustrating the schema. Persistence remains YAML-authoritative for v1 (accepted resolution, `.archive/specs/people.md` §5.5) with an evidence-gated re-evaluation trigger, not an immediate SQLite migration.
